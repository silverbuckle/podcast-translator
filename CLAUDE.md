...
python src/main.py "URL"                    # 通常（TTS あり）
python src/main.py "URL" --mode text        # テキストのみ（TTS スキップ）
```
出力: `output/audio/<hash>.mp3` or `output/scripts/<hash>.txt`
...
```
LAN 内の他デバイス（iPhone 等）からもアクセス可能（`0.0.0.0:8000`）
...
|------|----------|----------|
| 1. 音声DL + メタデータ | `download.py` | yt-dlp (Chrome cookies), feedparser, iTunes API |
...
src/claude_api.py        # Claude API 共通ヘルパー（ストリーミング・JSONパース・エラーハンドリング一元管理）
src/server.py            # FastAPI Web サーバー（ジョブ管理・ログキャプチャ・ファイル配信）
src/static/index.html    # Web UI（ダークモード、ローカル API 連携）
...
- Apple Podcast URL → iTunes Lookup API で RSS 取得 → エピソード音声ダウンロード
